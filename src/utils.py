...
def inverse_warp(imgs, pose, depth, intrinsics):
        ...
        indices = indices.flatten(start_dim=-2) # (1 x 3 x H*W)
        ...
        # homogenize
        depth_cam_coords_homo = torch.cat((depth_cam_coords, torch.ones((1, 1, h, w))), dim=1) # (B x 4 x H x W)
        ...
        warped_pixels = warped_pixels[:, :-1, :] / warped_pixels[:, -1, :].clamp(min=1e-3) # (B x 2 x H*W)
