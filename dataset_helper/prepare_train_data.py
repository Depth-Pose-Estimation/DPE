...
def dump_example(args, scene):
    ...
    for scene_data in scene_list:
        ...
        for sample in data_loader.get_scene_imgs(scene_data):
            ...
            dump_img_file.parent.mkdir(exist_ok=True)
            imwrite(dump_img_file, img)

            if "pose" in sample.keys():
                poses.append(sample["pose"].tolist())
            if "depth" in sample.keys():
                dump_depth_file = dump_dir/'{}_depth.npy'.format(frame_nb)
                np.save(dump_depth_file, sample["depth"])
                dump_depth_interp_file = dump_dir/'{}_depth_interp.npy'.format(frame_nb)
                np.save(dump_depth_interp_file, sample["depth_interp"])
        if len(poses) != 0:
            np.savetxt(poses_file, np.array(poses).reshape(-1, 12), fmt='%.6e')

        if len(list(dump_dir.glob('*.jpg'))) < 3:
            dump_dir.rmtree()


def main():
    args.dump_root = Path(args.dump_root)
    args.dump_root.mkdir(exist_ok=True)
    global data_loader
    ...
    subdirs = args.dump_root.absolute()
    canonic_prefixes = set([subdir.name[:-2] for subdir in subdirs.iterdir()])
    with open(args.dump_root / 'train.txt', 'w') as tf:
        with open(args.dump_root / 'val.txt', 'w') as vf:
            for pr in tqdm(canonic_prefixes):
                corresponding_dirs = list(subdirs.glob(f'{pr}*'))
