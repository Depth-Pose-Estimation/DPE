...
class KittiRawLoader(object):
    ...
    def collect_train_folders(self):
        ...
        for date in self.date_list:
            ...
            if drive_set.exists():
                for dr in drive_set.iterdir():
                    if dr.is_dir() and dr.name[:-5] not in self.test_scenes:
    ...
    def get_scene_imgs(self, scene_data):
        def construct_sample(scene_data, i, frame_id):
            ...
            if self.get_depth:
                sample['depth'], sample['depth_interp'] = self.get_depth_map(scene_data, i)
    ...
    def load_image(self, scene_data, tgt_idx):
        img_file = str(scene_data['dir']/'image_{}'.format(scene_data['cid'])/'data'/scene_data['frame_id'][tgt_idx])+'.png'
